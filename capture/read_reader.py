...
def store_message(message, elapsed_time):
    if csvfile is not None:
        csv_writer.writerow([elapsed_time, message.value*message.multiplier,
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "", ["csv=", "raw=", "rawtime=", "currentjson=", "serialport=",
                               "help", "filewait=", "threshold=", "base", "stop-on-threshold", "model=", "run-on-threshold="])
...
meterreader = reader.Reader(
    meter, message_handler=handle_message, filewait=debugwait, raw_handler=save_raw)
