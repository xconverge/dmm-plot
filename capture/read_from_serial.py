...
def store_message(message, elapsed_time):
    if save_csv:
        csv_writer.writerow([elapsed_time, datetime.now(), message.value*message.multiplier,
                            message.base_unit, message.mode, message.hold, message.rel])
        csvfile.flush()
    if save_rawtime:
...
def handle_message(message):
    ...
    global under_threshold, over_threshold
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "", ["csv=", "raw=", "rawtime=", "currentjson=", "debug=",
                               "serialport=", "help", "filewait=", "threshold=", "base", "stop-on-threshold", "model="])
...
if save_csv:
    ...
    csv_writer.writerow(["#Time [s]", "Value", "Unit",
                        "Modus", "Hold", "Relative"])

if not debug:
    serial_port = serial.Serial(
        portname, baudrate=2400, parity='N', bytesize=8, timeout=1, rtscts=1, dsrdtr=1)
...
while True:
    ...
    if MultimeterMessage.check_first_byte(test[0]):
        ...
        if save_raw:
            rawfile.write(test)
        print("received incorrect data (%s), skipping..." %
              test.hex(), file=sys.stderr)
        continue
    if len(data) != MultimeterMessage.MESSAGE_LENGTH:
        print("received incomplete message (%s), skipping..." %
