            ...
            if self.source.MultimeterMessage.check_first_byte(test[0]):
                data = test + \
                    self.serial_port.read(
                        self.source.MultimeterMessage.MESSAGE_LENGTH-1)
            else:
                self.print("received incorrect data (%s), skipping..." %
                           test.hex(), file=sys.stderr)
                continue
            if len(data) != self.source.MultimeterMessage.MESSAGE_LENGTH:
                self.print("received incomplete message (%s), skipping..." %
    ...
    def start(self):
        while True:
            ...
            if len(test) != 1:
                if self.source.type == "file":
                    return  # EOF
                self.print("recieved incomplete data, skipping...",
                           file=sys.stderr)
                continue
            if self.source.MultimeterMessage.check_first_byte(test[0]):
                data = test + \
            ...
            try:
                ...
                self.print("Error decoding: %s on message %s" %
                           (str(e), data.hex()))
                continue
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                print_thread_err("Error decoding: %s on message %s" %
                                 (str(e), data.hex()))
                if self.error_callback is not None:
                    self.error_callback(self.source)
                continue
