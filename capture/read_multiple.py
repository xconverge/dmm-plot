...
class Source:
    def __init__(self, path):
        ...
        if os.name == 'nt':  # running on windows
            ...
            # Character device, assuming serial port
...
class ReadThread(threading.Thread):
    def __init__(self, source):
        ...
        if source.type == "serial":
            self.serial_port = serial.Serial(
                source.path, baudrate=2400, parity='N', bytesize=8, timeout=1, rtscts=1, dsrdtr=1)
    ...
    def run(self):
        while True:
            ...
            # read one byte, used for determining if message is valid
            ...
            # check if first nibble is 0x1, if it isn't this is not the beginning of a valid message
            if (test[0] & 0b11110000) == 0b00010000:
                # looks good, read the remaining message
                data = test + self.serial_port.read(13)
            else:
                print_error(
                    "received incorrect data (%s), skipping..." % test.hex())
                self._delete_value()
                continue

            if len(data) != 14:
                print_error(
                    "received incomplete message (%s), skipping..." % data.hex())
                self._delete_value()
                continue
            ...
            try:
                ...
                print_error("Error decoding: %s on message %s" %
                            (str(e), data.hex()))
                self._delete_value()
                continue
...
def handle_messages():
    ...
    # we don't need more than 4 digits
...
valid_arguments = ["source=", "help", "filewait=",
                   "rate=", "csv=", "no-stdout", "manual"]
...
if csvfile is not None:
    if manual:
        csvwriter = csv.DictWriter(
            csvfile, ["time", "manual"]+[str(x) for x in sources], dialect="excel-tab")
    else:
        csvwriter = csv.DictWriter(
            csvfile, ["time"]+[str(x) for x in sources], dialect="excel-tab")
